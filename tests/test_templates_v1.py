...
def test_field_both_mapping_and_static_rejected(client):
    body = _payload()
    body["fields"][0]["static_text"] = "x"  # already has incident_mapping
    _assert_422(client.post(TEMPLATES_URL, json=body))


def test_field_no_data_source_rejected(client):
    body = _payload()
    del body["fields"][0]["incident_mapping"]  # neither mapping nor static
    _assert_422(client.post(TEMPLATES_URL, json=body))
